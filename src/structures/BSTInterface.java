...
public interface BSTInterface<T extends Comparable<T>> {
  ...
  /**
   * Attempts to remove one copy of an element from the tree, returning true if and only if such a
   * copy was found and removed.
   *
   * <p>The modified tree must still obey the BST rule, though it might not be balanced.
   *
   * @param element
   ...
   *
   * <p>The Iterator's behavior is not defined in the case that the tree is modified before the
  ...
  /**
   * Returns true if and only if this tree and the other tree have the same structure, and
   * equivalent values at each node. Uses equals() to check for node value equivalence.
   *
   * @param other
  ...
  /**
   * Returns true if and only if this tree and the other tree store the same values, regardless of
   * structure. Uses equals() to check for stored value equivalence.
   *
   * @param other
  ...
  /**
   * Returns true if and only if the tree is completely balanced. A balanced tree is either a full
   * tree, or a tree that would be full if all leaves in the highest level were removed.
   *
   * <p>An empty tree is considered balanced.
   *
   ...
   *
   * <p>The modified tree must still obey the BST rule, and must contain the same values it did
   * before it was balanced.
   */
  public void balance();

  /**
   * (This is a utility method that you would not include in a real BST's public interface. It's
   * here to help us grade and to help you write tests.)
   *
